...
class GameData:
    ...
    def setHead(self, cell):
        #splice list at beginning to add head cell, and change previous
        #cell to a body cell
    ...
    def getNextCellInDir(self):
        ...
        elif self.__currentMode == self.SnakeMode.GOING_NORTH:
    ...
    def reverseHelper(self):
        self.__snakeCells[0].becomeBody() #make head into body
        self.__snakeCells.reverse() #reverse the snakeCells array to make the tail end the head
        self.__snakeCells[0].becomeHead() #make the new 0 index (formerly the tail) the head

        if self.__currentMode == self.SnakeMode.GOING_NORTH: #reverse the direciton of the snake entirely based off the prev. direction 
            self.__currentMode = self.SnakeMode.GOING_SOUTH
        elif self.__currentMode == self.SnakeMode.GOING_SOUTH: 
            self.__currentMode = self.SnakeMode.GOING_NORTH
