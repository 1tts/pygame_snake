...
class BoardCell:
    ...
    def __lt__(self, other):
        """ overriding the built in __lt__ func from python. Needed when the f(n) value
        between 2 tuples in the Prio queue (for the A* search) are the same, and python
        defaults to comparing the two BoardCell objects. without this, i would get a typeError
        (because how are you gonna do BoardCell1 < BoardCell2, makes no sense), and this allows
        for the comparison to run based off of the row, col values of the two cells """

        if self.__row != other.getRow():
            return self.__row < other.getRow()
        else:
            return self.__col < other.getCol()
    ###########################################
    # Access basic information about the cell #
    ###########################################
    def getGCost(self):
        """return the g(x) weight of A* algorithm """
        return self.__gCost
    ...
    def setGCost(self, value):
        """ set the g(x) weight for A* search algorithm """
        self.__gCost = value
